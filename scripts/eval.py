...
CASES = [
    ...
    Case("reminder.create", "remind me to call mom at 9pm today", [["9:00 PM"], ["Reminder set"]]),
    Case("reminder.duplicate", "set a reminder to call mom at 9pm", [["Already set"], ["didn't add a duplicate"]]),
    Case("reminder.list", "any reminders?", [["call mom"]]),
    Case("reminder.cancel", "cancel my reminder", [["Cancelled", "which should I cancel"]]),
    Case("calendar.create.ask", "add event 'eval test' tomorrow 11am to my calendar",
         [["About to create a calendar event"], ["11:00 AM"]]),
    ...
    Case("home.nosensor", "kitchen temperature?", [["no sensor in the kitchen"]]),
    Case("home.switch.ask", "turn off the AC", [["About to turn the AC OFF"]]),
    Case("home.switch.decline", "no", [["cancelled", "nothing was done"]]),
    Case("email.metadata", "any new emails?", [["unread", "Couldn't check email"]]),
    Case("email.boundary", "open the first email", [["can't open email contents", "Couldn't check email"]]),
    Case("guard.pastevent", "add event 'old' on 22 jan 2024 3pm to my calendar",
         [["in the past", "couldn't work out"]], must_not=["reply \"yes\""]),
