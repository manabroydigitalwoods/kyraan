...
| 2 | **House steward** | purifier filter life (20% / 10%, once each), AC energy by month from a nightly ledger (compared on the 1st), a 21:45 settle check before quiet hours (AC drawing, purifier on turbo with no timer, display light on — each with its one-line fix; silent when settled); "house status" on demand | 21:45 nightly; the 1st | **built 2026-09-03** (`triggers/house_steward.py`) |
