...
Owner, 2026-09-03: "the centre is cluttered — people are there; make it readable, right spaces and positions for each group." Two causes measured: five lobes anchored within 0.6 of the core (people 0.2, notes 0.89, faces 1.01 — the closest pair, note–face, 0.43 apart), and all nine people shared one anchor while the owner's 251 wires dragged it to 0.94 from the core with the rest at 0.3–0.5. Now: `LOBES` puts every non-person lobe ~1.15 out on the sphere with no two neighbours sharing a side; people are a RING (`anchorFor`: each person has its own point at radius 0.42, evenly spaced) and ring nodes ease back to their point every step — the ring is the layout, the wires bend toward it (a dragged person is exempt while dragged). On a portrait canvas the layout turns 90° and stands up (`brain.portrait` from `sizeCanvas`, reheat on change). After: all nine people at 0.42, min person spacing 0.24 → 0.29, closest lobes 0.43 → 0.57 (note–tag, deliberately adjacent), drawn span 300×354 on a 359×652 canvas.

Folded turn card (owner: "collapsible popup for mobile, single line visible, open if the user wants"): on a phone the card starts folded to its head line — turn id, “what was asked”, the latest step — and a tap on that line (or the ▾) opens it; the choice sticks for the session and the desktop starts open. Measured: 29px folded, 212px open for a five-step turn, 29px again after folding.

## Every pulse is a round trip from the core

Owner, 2026-09-03: "the pulses should fire from the centre, then come back to the centre — finally Kyraan." Before, a thinking pulse started at the person and a tool pulse at the skill; only the call wires touched the core. Now a pulse is a ROUTE: a path of node ids starting at the core, walked hop by hop along the wires as drawn, then walked back (`emitRoute`, `routeSegment`, the route branch in `advanceSignals`). Thinking = core → person → fact → person → core on the subject wires (up to four, staggered); recall = core → person → episode and home on the spoke wires; a tool ask = core → skill (and its co-fired neighbours, core → skill → other and home); the result = skill → core; the reply = core → person and home; a reminder = core → task and home. Between two nodes with no stored wire (the core and a person without a `talks` edge) the pulse rides a transient call wire, never empty space. Proof with one synthetic turn: 41 segments in 22 chains; every chain leaves the core; 11 outbound legs each have their return leg ending at the core.

## Catching the panel up with a week of Kyraan (2026-09-04)

Owner: "recently added lots of things — did we update the web panel and brain also?" Audit against the commits since 2026-09-02: (1) `wikilink` — the server emits it for note↔note Obsidian links since "wikilinks are hard edges"; the client had no style, so `showEdge` was undefined and the edges were silently skipped — style added. (2) Coding tasks (Claude Code in a worktree) live in `data/code_jobs.json`, not the trigger board; they now join the work lobe as `t:code:<id>` with status, branch, `owns` to whoever asked (registry chat id) and `managed_by` to the four `code.*` skills; the Selection card shows status and branch. (3) The four duties (Kiaan's keeper, house steward, chief of staff, whereabouts) and brief failures now land on the overview timeline by name. (4) Private mode needs nothing: the trace log's redaction placeholder is what the panel reads. (5) The panel is `ai.kyraan.panel` (launchd, `--lan`); restart with `launchctl kickstart -k gui/$(id -u)/ai.kyraan.panel`, never a hand-started second process (it took port 8765 from the service once today). Not yet drawn: whereabouts places (`data/duties/whereabouts.json` — `places` is still empty), Kiaan's keeper state (file not created yet). Live after restart: the one discarded coding job with 5 wires; edge kinds 17.

Short-term memories (owner, 2026-09-04: "show the short-term memories in the brain?"): they were there — 18 of 52 facts have `term = short` — but the brain dropped the term the map already knew, so nothing told them apart. Now every memory node carries `term`; a short-term fact is drawn HOLLOW (held, not kept — it fades 14 days after it was learned, memory/engine._SHORT_TERM_DAYS), its card says when ("term: short — forgotten 2026-09-14 (11d left)", flagged red inside three days), and the colour picker gains a `term` mode. Test: every memory neuron says its term.
...
Stage three — semantic zoom for the recall lobe. 212 episodes were the hairball at the centre and the lobe grows ~15 a day (the browser wall we measured at ~1,500 nodes). Zoomed out, episodes fold into their DAYS: one neuron per day ("26 Aug · 102"), sized by log of its count, placed at the centroid of its exchanges; wires from folded episodes are drawn to the day once each (`standIn`); routes to a folded episode arrive at its day; a day's focus and callout carry its exchanges' wiring ("9 wires", lights 10 of 202). Fold below 1.15×, open above 1.35× (hysteresis), only past 60 episodes. Double-tap or "open this day" frames the day's exchanges, which opens it. Measured on a phone at the fit scale (0.80): 212 episodes → 11 days, 403 → 202 visible neurons; the census and caption still say 212. The day neurons are not simulated and carry no edges; they are a view, not data.

## A brain, not a starburst

Owner, 2026-09-04: "make it more like a human brain, and don't make a dense single point." The dense point was 106 structural wires (acts 70, received 32, fires 3, talks 1) ending on the core with the people ring on top of it. Three changes. (1) Tracts: a wire out of the core bends through the centre of the lobe it goes to (`edgeControl` with the two ids; `brain.lobeScreen` is filled by the caption pass each frame), so seventy wires to the skills leave the core as one nerve and fan out inside the lobe — pulses and call wires ride the same curve; the core's resting wires are dimmer (acts 0.14, received 0.07). (2) A keep-out: inside 0.45 of the origin any non-ring neuron is pushed back out; the people ring widens to 0.55. (3) The anchors are a brain seen from the side — skills and work forward (frontal), facts on top (parietal), recall out to the side (temporal), documents and faces at the back (occipital), notes and tags below-behind (cerebellum), contacts at the stem; canvas y grows downward, so "up" is negative (the first cut drew the facts at the bottom). Measured on a 1115×712 canvas: 0 neurons inside the keep-out, all ten people at 0.55, closest lobes 0.64 (note–tag, adjacent by design), lobe screen heights facts 18% → skills 36% → recall/documents ~50% → faces/notes 77% → tags 85% → contacts 90%; a core→skill wire's control point is the 25/75 blend with the skills centre.

Second cut, the cortex (owner: "still not like a human brain cluster"). Islands in space are a molecule, not a brain; a brain is one oval mass, two hemispheres, a fissure. So: every neuron but the core and the people ring is held ON an ellipsoid shell (`CORTEX`, a 1.05 × b 0.80 × c 0.78, between 0.70 and 1 of it, snapped each step — half-strength let the springs win and left 113 of 396 inside), pushed off the midline by 0.12 so a fissure runs down the middle, and every lobe is anchored ON the midline so the fissure deals it to both hemispheres (seeds alternate sides): 199 left / 197 right, every lobe split (recall 109/106, skills 34/36, facts 27/25). Repulsion 0.00011 and anchor 0.08 for a packed cortex; the region halos are wider and brighter so the regions read as one glowing mass; the recall lobe folds only below 0.95× so a desktop fit (1.35×) shows the 215 exchanges as the temporal mass instead of 11 dots. Home camera is the lateral view (yaw −1.25, pitch 0.22): facts and skills as the crown, documents at the back, recall as the temporal bulge, notes/tags/contacts below. Still nothing decorative: no outline, no texture — the shape is the evidence's positions.

Third cut (owner: "looks good but can be more improved"): wires run OVER the cortex. A non-core wire's control point is its 3D midpoint pushed out to just above the shell (longer wire, higher arc, capped at 1.35×), projected with the frame's canvas and view (`brain.frame`), so a synapse between two facts arcs across the surface like a gyrus instead of cutting a chord through the middle — pulses ride the same arc. The cortex band thins to 0.80–1.0 (396/396 in it), and the far hemisphere fades harder (depth^0.7, soma alpha to 38% at the back) so the near one reads in front. Proof: a synapse's control point sits 333px from the core against 313px for its chord midpoint.

Fourth cut (owner: "all groups are cluttered, make every group clearly visible"). Three changes on the same shell: region tension — a neuron repels a neighbour from ANOTHER lobe 2.4× harder than its own (0.85×), so regions keep clear edges instead of bleeding; anchors re-spaced by lobe size (recall 215 low across both sides, skills 70 front-top, facts 52 the crown, documents 25 back-top, the small lobes at the back and stem); and cross-lobe wires rest at 40% so their arcs stop smearing two regions together (full when touched). Measured, 800 hand-driven steps: 3D purity (share of a neuron's 6 nearest that are its own lobe) 0.95 → 0.97; on-screen mixing 10%; closest centroids note–tag 0.24 → 0.27.

Fifth cut (owner: "only two sides are used — use 360°, more readable and manageable"): anchoring every lobe on the midline and splitting it with a groove had made two flat sheets with a hollow between them (the front view showed it). Now the ten regions sit on the vertices of an icosahedron scaled onto the shell — facts top-back, work top-front, skills front-one-side, documents back-one-side, faces back-other-side, recall low on one flank, notes low on the other, contacts bottom-front, tags bottom-back — with no fissure and a three-quarter home view. Measured: all 8 octants occupied (from 7 to 125 neurons each), closest region centroids 0.64 apart (tag–face), purity 0.99. From the front, the side and the three-quarter view every region reads as its own block.

Sixth cut (owner: "in this position the cluster behind makes the front less readable"): the far hemisphere printed through the near one. `backFade`: camera-space depth eases from 1 at −0.12 (in front of the centre plane) to 0.2 at +0.32 (behind it), applied to somas, wires (as far back as their further end), region halos and captions; far-side labels are skipped unless asked for. Measured at the owner's angle: 160 neurons behind the plane at 20%, 93 in front at 100%, the centre at 85%. The globe now reads as opaque — turning it brings a region forward and sends the one behind back — with nothing removed: everything behind is still there at a fifth.

## Six suggestions, taken (2026-09-04)

Skip, not fail: the six panel tests that read the live store now take a `real_pg` fixture that pings Postgres with a two-second statement timeout and skips with the reason when it cannot — under load they had failed and read as regressions. The mesh moves to pgvector: `_synapses_pg` asks the store for each row's k nearest (`<=>`, a lateral join) with the same top-3 / floor / once-per-pair rule, for facts and episodes; numpy stays as the fallback and for the document cross-mesh. Verified on the live store: 52 facts → 52 edges from both, identical sets, 10ms against 48ms — and no n×n matrix anywhere. Two lobes for the duties: `place` (whereabouts: a remembered place with radius and since; `at` to the owner while a visit says inside; `mentions` from documents naming it) and `care` (Kiaan's doses from the keeper's own `done_map`/`upcoming`: done with date and source, or due-soon/overdue/later; owned by the child) — both on the icosahedron's two spare vertices, both empty today, tested with fakes.

## Phase C, the review queue (2026-09-04)

The brain's Findings console now acts, on exactly two things it surfaces: an orphan fact gets "forget" (`memory.engine.forget([id])` — deactivated, kept as history, the engine logs `memory_forgotten`), and a `maybe` contact gets "is <person>" (`store.persons.add_alias(person, contact name)` — the dashed wire becomes an exact `is` on the next build). `POST /api/review/forget` and `/api/review/confirm_contact` (server `do_POST`, `panel/actions.py`). Rails: the cookie (the owner), a header only the panel's own script sends (`X-Kyraan-Panel: review` — no cross-site form can add one), an Origin check against the Host allowlist, the kill switch (423 while engaged), ids and names the graph itself handed out (no free text), a `panel_review` audit event per action, a browser confirm per click, and the graph memo dropped after a change. The never-writes rule still holds for every GET (its test is untouched). Tests: 403 without the header or from a foreign Origin, 401 with a bad token, 423 at the kill switch (engine never called), forget runs the engine for one id and drops the memo, confirm adds the alias lower-cased. Client proof with a stubbed fetch: 19 buttons on today's findings; clicks produced `POST /api/review/confirm_contact {"person":"kamal","name":"Habu Friend"}` and `POST /api/review/forget {"fact_id":"16876bd9"}` with the header, then a graph refresh. Not offered: "keep" for a short-term fact — no tool promotes a term today.

Left with the owner: `git push` (this session's permission classifier refused it) and Tailscale (not installed on the Mac; once it is, the service binds its address and the LAN exposure goes).

## Exploring without losing your place (2026-09-04)

Owner: "when I select a neuron and then rotate or zoom to see it clearly, it gets unselected and is lost." Two causes. On a mouse, the browser fires `click` after mousedown+mouseup on the same element even when the pointer travelled across the canvas: every orbit ended as a click on empty space, which cleared the selection — a press that moved more than 4px is now a drag, not a click. On a phone, when the second finger of a pinch lifted first, the last finger's lift read as a tap on nothing — a gesture (two fingers, or one finger moved past 8px) is remembered until every finger is up, and a tap is only a tap when there was no gesture. And the lost feeling: the view did not follow the choice. `holdSelection` keeps the selected neuron where it was on screen through any orbit or zoom (so you orbit around it and zoom into it); a pan or a fit moves it deliberately and re-bases the lock; the idle spin pauses while anything is selected. Measured: selection kept after a drag-click and after a pinch; orbit drift 0px, zoom drift 0px, a pan moves it by the pan; spin off while selected, back when cleared.

Inertia (owner, 2026-09-05: "touch or click and swipe, and it keeps turning with physics and slowly stops"): a drag's velocity is tracked while it is down (smoothed, in rad/ms or px/ms); a release within 80ms of the last move starts a coast at that velocity, capped at 6 rad/s (3 px/ms for a pan), decaying 0.994 per ms — a firm flick adds about a radian over a second — and any new press stops it dead. Mouse and touch, orbit and pan; the selection lock holds through the coast; the idle spin waits for it. Two harness lessons: two mouseup listeners see one event, so a release with nothing to release must not clear a coast it did not make; and a background tab throttles timers to a second, so the release window is verified with busy-waits. Measured: 120px flick → −6 rad/s, 1.31 rad, 976ms; 30px flick → 1.11 rad; press stops it; selection kept.

Turn per pixel follows the zoom (owner, 2026-09-05: "full zoomed, the movement feels very fast"): `turnGain` = 1 / max(1, scale), applied to orbit on mouse and touch (and so to the fling). Measured: a 60px drag turns 0.132 rad at 0.8× and 0.041 rad at 3.2×, ratio 0.313 = 1/3.2, so what is under the finger moves at about the same screen speed at any zoom.
